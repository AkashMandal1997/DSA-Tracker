...
function Topic({ data, updateData }) {
                    ...
                    question: (
                        <>
                            <a
                                ...
                                // style={{ fontWeight: "600", fontSize: "20px", backgroundColor: "white" }}
    ...
    function displayToast(isSelect, id) {
        let message;
        if (isSelect) {
            message = "Congrats! You have completed the task.";
        } else {
            message = "Unfortunately, it seems that the task has not been completed";
        }

        toast(message, {
            className: `toast-${isSelect ? "Done" : "Incomplete"}`,
            autoClose: 2000,
            closeButton: true,
        });
    }
    ...
    return (
        <>
            <br />
            <h1 className="text-center mb-4">{topicName} Problems</h1>
